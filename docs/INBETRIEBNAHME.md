...
| **Ollama-/whisperX-Container** | geplant (Dockerfiles mit Build-Pre-Flight-Healthcheck), kein GPU-Workload im Image | laufender Upstream-Ollama (Prod `192.168.178.11:11434`, Dev `localhost:11434`) + whisperx-mcp (`localhost:8099`). `config/speech.php` `WHISPER_URL`/Port-Kollision 8099↔`APP_PORT` angleichen; `OLLAMA_MODEL`-env → ModelRouter. Plan: `docs/ai-services-plan.md` |
| **VLM-Beleg-Capture** | geplant — Backend-Logik (Foto→Analyse→Vorschlag→berechtigte Bestätigung), Fake-VLM-Adapter (`SPEECH_FAKE`-Muster) für dev/test | Ollama-VLM-Modell verfügbar + `Befugnis`-Tätigkeit `beleg_einsortieren`. Plan: `docs/ai-services-plan.md` |
