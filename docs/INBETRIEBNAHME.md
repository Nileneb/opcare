...
## 6. Abstimmungen / Wahlen (Voting-Modul)

| Schalter | Default | Bedeutung |
|---|---|---|
| **`voting.online_wahl_aktiv`** (`.env`: `VOTING_ONLINE_WAHL`) | `false` | Bindende Online-**Wahlen** (Art = `wahl`) sind gesperrt; Umfragen und Beschlüsse laufen uneingeschränkt. Aktivieren sobald der Inbetriebnahme-Prozess (ggf. Datenschutz-Prüfung, Testphase) abgeschlossen ist. |
| **`voting.krypto_unverkettbarkeit_aktiv`** (`.env`: `VOTING_KRYPTO_UNVERKETTBARKEIT`) | `false` | **Gebaut & stillgelegt.** Der Stimmodus `GeheimKrypto` (Server-/Root-Unverkettbarkeit) ist als echter Schalter vorhanden: Enum-Case, Service-Sperre (`AbstimmungStarten` + `StimmeAbgeben`) und UI-Ausblendung greifen bereits. **Stillgelegt, weil die Krypto-Härtung selbst noch fehlt** — die heutige Anonymität ist „Trennung + Zugriffskontrolle + UUID/timestamp-frei" (DSGVO ErwG 26), aber NICHT kryptografisch gegen einen Root-Admin unverkettbar. **Aktivieren erst nach Implementierung** des blind-signierten Token-Flusses (Wähler holt blind-signierten Berechtigungstoken, gibt ihn beim Einwurf entkoppelt ab → Server kann Stimme↔Wähler nicht mehr verketten), dann `VOTING_KRYPTO_UNVERKETTBARKEIT=true`. Bis dahin lehnt der Code den Modus an der Quelle ab — es wird **nichts** vorgetäuscht. Details: `docs/abstimmungen.md`. |

**Scope dieser UI:** Mitarbeitende und Gremiumsmitglieder mit User-Login können über `/abstimmungen` abstimmen.

**Bewohner-Wahlen (Elektorat = Bewohner):** Bewohner haben i. d. R. keinen eigenen Login. Die
Wahlteilnahmen werden zwar angelegt (resident_id-basiert), aber die eigentliche Stimmabgabe über diese
Oberfläche ist auf User-basierte Wähler beschränkt. Ein **Bewohner-Kiosk-Pfad** (assistierte Stimmabgabe,
z. B. auf einem Tablet mit Kiosk-Session) ist als Folge-Inkrement vorgesehen und hier bewusst noch nicht
implementiert.

**config-Eintrag:** `config/voting.php` → Schlüssel `online_wahl_aktiv` (.env `VOTING_ONLINE_WAHL=false`)
und `krypto_unverkettbarkeit_aktiv` (.env `VOTING_KRYPTO_UNVERKETTBARKEIT=false`).

---

## 7. CI-Gates, die extern-gegatet „skippen" (kein Fehler, sichtbar)
